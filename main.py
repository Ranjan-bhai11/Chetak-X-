...
# 🚨 REAL METALLIC SHARP STEALTH CAR FRAME (NO SOUND - 15 SECONDS EXACT)
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Poppins', sans-serif; }
        ...
        /* 🚨 15-SEC HIGH-TECH STEALTH OVERLAY */
        #splash-screen { 
            position: fixed; 
            inset: 0; 
            background: #000000; 
        ...
        .splash-sub { font-size: 10px; color: #555; letter-spacing: 4px; text-transform: uppercase; margin-bottom: 60px; }

        /* Real Aggressive Sharp Silhouette Geometry Front Panel */
        .car-stealth-chassis { 
            width: 320px; 
            height: 150px; 
            position: relative;
            background: linear-gradient(180deg, #09090b 0%, #020202 100%);
            border-radius: 6px;
            border-bottom: 3px solid #1a1a1a;
            box-shadow: 0 25px 60px rgba(0,0,0,0.95);
        }

        /* Front Bonnet Line Grid */
        .bonnet-crease {
            position: absolute;
            top: 0;
            left: 50%;
        ...
        /* Exact Aggressive Lower Flattened Red Angel Eyes */
        .laser-eye {
            position: absolute;
            top: 60px;
            width: 65px;
        ...
        /* Slanted cutting-edge profile matching the real photo */
        .eye-left { left: 20px; transform: skewY(-6deg) rotate(-4deg); border-radius: 2px 8px 2px 4px; }
        .eye-right { right: 20px; transform: skewY(6deg) rotate(4deg); border-radius: 8px 2px 4px 2px; }
        
        /* Premium Dual Section Dark Matte Grill */
        .twin-grill-deck {
            position: absolute;
            bottom: 35px;
            left: 50%;
        ...
        /* High-Intensity Precision Glow Activation */
        #splash-screen.ignited .laser-eye {
            background: #FF1E1E;
            box-shadow: 0 0 35px #FF1E1E, 0 0 15px rgba(255, 30, 30, 0.9);
            border-bottom-color: #FF5555;
        }
        #splash-screen.ignited .twin-grill-deck {
        ...
        .tap-hint { font-size: 9px; color: #444; letter-spacing: 1px; margin-top: 10px; text-align: center; }
...
<body>
    ...
    <div id="splash-screen" onclick="igniteEngine()">
        ...
        <div class="car-stealth-chassis">
            <div class="bonnet-crease"></div>
            <div class="laser-eye eye-left"></div>
            <div class="twin-grill-deck">
                <div class="grill-wing"></div>
                <div class="grill-wing"></div>
            </div>
            <div class="laser-eye eye-right"></div>
        </div>
        <div class="init-text" id="status-label">TAP TO START INTERFACE</div>
        <div class="tap-hint" id="hint-label">(Tap once to trigger the 15-second authorization sequence)</div>
    </div>
    ...
    <script>
        ...
        function igniteEngine() {
            ...
            status.innerText = "AUTHENTICATING STEALTH MATRIX...";
            splash.classList.add('ignited');

            // 🛠️ EXACT 15 SECONDS TRANSITION LOOP CONTROL
            setTimeout(() => {
                status.innerText = "LOADING SECURE CONSOLE ENGINE...";
                setTimeout(() => {
        ...
        async function sendMessage() {
            ...
            try {
                ...
                errWrap.innerHTML = `<div class="message ai-message">⚠️ System error! Response verification failed.</div><span class="msg-time">${timeStr}</span>`;

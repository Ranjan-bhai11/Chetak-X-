...
# 🏎️ FULL PREMIUM UI WITH 5-SEC SPLASH SCREEN & V8 EXHAUST AUDIO
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Poppins', sans-serif; }
        ...
        .splash-car-box { 
            ...
            max-width: 420px; 
            height: 260px; 
            background: url('https://images.unsplash.com/photo-1555215695-3004980ad54e?q=80&w=600&auto=format&fit=crop') center/cover; 
            position: relative; 
            border-radius: 12px;
            box-shadow: 0 0 50px rgba(0,0,0,1) inset;
        }
        
        /* Smooth Red Eyes Opening Animation */
        .splash-eyes { 
            position: absolute; 
            inset: 0; 
            background: radial-gradient(circle at 35% 55%, rgba(255,0,0,0) 0%, transparent 100%); 
            transition: background 4.5s cubic-bezier(0.4, 0, 0.2, 1); 
            pointer-events: none;
        }
        
        #splash-screen.ignited .splash-eyes { 
            background: radial-gradient(circle at 34% 53%, rgba(255,0,0,0.7) 0%, transparent 12%), 
                        radial-gradient(circle at 66% 53%, rgba(255,0,0,0.7) 0%, transparent 12%); 
        }
        
        .splash-logo { font-size: 28px; font-weight: 700; letter-spacing: 4px; color: #FFFFFF; margin-top: 30px; text-shadow: 0 0 10px rgba(255,255,255,0.1); }
        .splash-logo span { color: #FF1E1E; }
        .splash-status { font-size: 10px; color: #FF1E1E; letter-spacing: 3px; margin-top: 8px; font-weight: 600; opacity: 0.6; animation: pulse 1.5s infinite; }
        ...
        /* Banner inside Chat Layout */
        .hero-banner { width: 100%; height: 200px; position: relative; background: linear-gradient(180deg, rgba(13,13,13,0.1) 0%, #0D0D0D 100%), url('https://images.unsplash.com/photo-1555215695-3004980ad54e?q=80&w=600&auto=format&fit=crop') center/cover; border-bottom: 1px solid #1A1A1A; flex-shrink: 0; }
        .hero-banner-overlay { position: absolute; inset:0; background: radial-gradient(circle at 34% 53%, rgba(255,0,0,0.3) 0%, transparent 12%), radial-gradient(circle at 66% 53%, rgba(255,0,0,0.3) 0%, transparent 12%); pointer-events: none; }
        ...
        /* EXACT NOT ROTATED RED SEND BUTTON */
...
<body>
    ...
    <!-- 🚨 CHETAK X ENGINE INITIALIZING SPLASH INTERFACE -->
    <div id="splash-screen">
        <div class="splash-car-box">
            <div class="splash-eyes"></div>
        </div>
        <div class="splash-logo">CHETAK <span>X</span></div>
        <div class="splash-status">INITIALIZING V8 TWIN TURBO...</div>
    </div>

    <!-- MAIN CONSOLE LAYOUT FRAMEWORK -->
    ...
    <script>
        // 🔊 SPLASH SCREEN TRIGGER WITH V8 ENGINE SOUND EFFECTS
        window.addEventListener('DOMContentLoaded', () => {
            const splash = document.getElementById('splash-screen');
            const mainApp = document.getElementById('mainApp');
            
            // Raw V8 Audio Startup source
            const v8Audio = new Audio('https://actions.google.com/sounds/v1/transportation/sports_car_rev_and_pull_away.ogg');
            v8Audio.volume = 0.7;

            // Step 1: Ignite the headlights & sound together after 300ms
            setTimeout(() => {
                splash.classList.add('ignited');
                v8Audio.play().catch(e => console.log("Audio requires user tap sometimes"));
            }, 300);
            
            // Step 2: 5 Seconds Hold, then clean fade out
            setTimeout(() => {
        ...
        async function sendMessage() {
            ...
            try {
                ...
                errWrap.innerHTML = `<div class="message ai-message">⚠️ System error! Server check karo.</div><span class="msg-time">${timeStr}</span>`;

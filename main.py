...
# 🏎️ 15 SECONDS EXTENDED SPLASH + HYPER V8 ENGINE ROAR AUDIO BYPASS
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Poppins', sans-serif; }
        ...
        /* 🚨 15-SEC PREMIUM STEALTH DASHBOARD SPLASH OVERLAY */
        #splash-screen { 
            position: fixed; 
            inset: 0; 
            background: #000000; 
        ...
        .splash-main-logo { font-size: 36px; font-weight: 700; color: #FFFFFF; letter-spacing: 6px; margin-bottom: 5px; text-align: center; }
        .splash-main-logo span { color: #FF1E1E; }
        .splash-sub { font-size: 10px; color: #555; letter-spacing: 4px; text-transform: uppercase; margin-bottom: 50px; }

        /* Aggressive Custom Cyber-Car Grill Silhouette Design */
        .car-vector-frame { 
            width: 300px; 
            height: 140px; 
            position: relative;
        ...
        /* Tikhi Angular Angel Eyes Framework */
        .angel-eye {
            position: absolute;
            top: 55px;
            width: 45px;
        ...
        .eye-left { left: 35px; transform: rotate(-12deg); }
        .eye-right { right: 35px; transform: rotate(12deg); }
        
        /* Stealth Center Aggressive Double Grill Line */
        .grille-center {
            width: 70px;
            height: 35px;
        ...
        .grille-bar { width: 30px; height: 100%; border: 1px solid #222; border-radius: 6px; }

        /* Ignited Glowing Neon Vibe */
        #splash-screen.ignited .angel-eye {
            background: #FF1E1E;
            box-shadow: 0 0 35px #FF1E1E, 0 0 15px rgba(255, 30, 30, 0.9);
        }
        #splash-screen.ignited .grille-center {
            border-color: #FF1E1E;
        ...
        .tap-hint { font-size: 9px; color: #444; letter-spacing: 1px; margin-top: 10px; text-align: center; padding: 0 20px; }
        ...
        /* Exact Layout Control bar input matching screenshot */
...
<body>
    ...
    <div id="splash-screen" onclick="igniteEngine()">
        ...
        <div class="tap-hint" id="hint-label">(Tap screen once to roar the V8 M4 engine)</div>
    </div>
    ...
    <script>
        ...
        function igniteEngine() {
            ...
            // 🔊 Bypassed HTML5 Native Web Audio Context (Ensures loud sports car rev sound)
            const audioCtx = new (window.AudioContext || window.webkitAudioContext)();
            const v8Audio = new Audio('https://actions.google.com/sounds/v1/transportation/sports_car_rev_and_pull_away.ogg');
            const source = audioCtx.createMediaElementSource(v8Audio);
            source.connect(audioCtx.destination);
            
            status.innerText = "IGNITING V8 TWIN TURBO...";
            splash.classList.add('ignited');
            
            // Play verified stream
            v8Audio.play().catch(e => console.log('Audio Context Bypass active'));

            // 🛠️ TIMING EXTENDED TO 15 SECONDS EXACTLY AS REQUESTED
            setTimeout(() => {
                status.innerText = "SYSTEM INJECTED. OPENING CONSOLE...";
                setTimeout(() => {
        ...
        async function sendMessage() {
            ...
            try {
                ...
                errWrap.innerHTML = `<div class="message ai-message">⚠️ Glitch! Dubara message bhejo bhai.</div><span class="msg-time">${timeStr}</span>`;
